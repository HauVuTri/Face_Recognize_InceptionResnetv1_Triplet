...
def create_model_triplet(input_shape, model, margin=0.5):
    ...
    model_triplet = tf.keras.Model(inputs=[anchor_input, positive_input, negative_input], outputs=output, name="Trainer_Model")

    return model_triplet

# Gọi model InceptionResNetV1
model = InceptionResNetV1()

# call model training with triplet loss
trainer_model = create_model_triplet((160, 160, 3), model, margin = 0.5)
trainer_model.compile(optimizer = Adam(lr=0.001), loss=None)
trainer_model.summary()

# Lấy data từ folder
data_train = get_data('DATA_TRAIN')

trainer_model.fit_generator(data_train, epochs=50)
